...
function query($sql, $noTransform = false)
{
	...
	if (!$con)
	{
		$con = @mysql_connect($GLOBALS['DBDATA']['host'], $GLOBALS['DBDATA']['user'], $GLOBALS['DBDATA']['password']);
		mysql_select_db($GLOBALS['DBDATA']['database'], $con) || die('Fehler, keine Datenbank!');
	}
